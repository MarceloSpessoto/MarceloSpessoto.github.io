...
During my GSoC project of planning a Jenkins CI for kworkflow, I've spent plenty of time experimenting with different possibilities of agent types for the infrastructure.
...
The use of agents to exclusively run jobs is also recommended for being a safer approach, as it gives appropriate controller isolation.
...
My first attempt of configuring an agent for Jenkins was by using the Docker Pipeline Plugin. It doesn't help to set up a Cloud or an agent itself, avoiding the cloud vs. static node classification
previously stated, but allows one to define in the **jenkinsfile** the Docker image that will be run for a specific pipeline/stage and dynamically use it in the pipeline when required.

It is pretty simple to use. You just have to install the plugin for your Jenkins server and write a jenkinsfile invoking the dynamic docker container. You can pull a container image from docker hub
defining 
...
[There is a full documentation on how to define docker agents from pipeline with the `docker-workflow` plugin with docker and dockerfile agent options](https://www.jenkins.io/doc/book/pipeline/syntax/#agent).
...
When trying to check if i could run kworkflow's integration tests (which requires creating new containers) with this plugin, I've attempted to run the agent with the `--privileged` option, a fix that allows Docker in Docker (DinD).
A very cursed and forbidden fix, that allows a container to have control over the host kernel.

When it actually worked, I've noticed that it wasn't very good news actually. Since this Jenkins CI will be used in an open source software context, exposing to every unknown contributor the ability to modify the jenkinsfiles
...
one could still modify the jenkinsfiles from other tests, such as code coverage, unit tests, etc...
...
So it felt wiser to not have this plugin installed on the server, disabling the possibility to define the agents from public Pipeline script.
...
2. have an user named "jenkins"

The [docker-ssh-agent container](https://github.com/jenkinsci/docker-ssh-agent) already fills these requirements and
is very recommended for container SSH agents. For other types of node, such as a VM, you will have to ensure it 
yourself.

And then, to make it work with the controller node, one must:
1. configure a SSH connection method for the controller to execute the jobs on the agent. This can be done by giving the agent a public
...
the launch method (in this case, a SSH agent, with the SSH credential to be used (we defined the in the 
previous code block), the host, port and SSH verification strategy).

You can get a fully detailed explanation of how to configure a SSH agent on the [Plugin official documentation](https://github.com/jenkinsci/ssh-agents-plugin/blob/main/doc/CONFIGURE.md).
...
To pass the custom port from host to the container, I've passed it to docker-compose.yml as an `ARG` and in the
Dockerfile I've passed the port `ARG` to and environment variable with `ENV`. The differences between Docker `ARG`s and `ENV`s can be seen in [this nice post](https://vsupalov.com/docker-arg-env-variable-guide/).

With the jenkins/ssh-agent, the public ssh key can be easily configuring by just setting the "JENKINS_AGENT_SSH_PUBKEY" env var, something I've done 
...
I've created a very simple Vagrantfile configuring the Java installation, creation of `jenkins` user, creating a directory for job execution and injecting the SSH public key. This can be done with [Vagrant provisioning]
(https://developer.hashicorp.com/vagrant/docs/provisioning). You can provision the VM with a automation tool like Ansible, Chief or Puppet. The basic shell provisioning was enough for my case.
...
Since docker-compose isolates its orchestrated containers in their network from the docker interface, it was necessary
...
After trying both Docker Pipeline Plugin and Docker SSH agents and not being satisfied with both, I've considered trying a Cloud for Jenkins with Kubernetes plugin. It would be much more complex and difficult to handle, but it
...
It would be a long path to follow and an overkill solution for the kworkflow, that doesn't contain such high amount of PRs being opened and could benefit from something simpler. Gladly, I've found out that I've been misled by
the confusing Docker plugins: there are actually two different Docker Plugins. 

I've installed both `docker-workspace` and `docker-plugin` before, but I've thought that the latter complemented the former as a solution for Pipeline Syntax for Docker use. Turns out that the `docker-plugin` has a completely
different purpose: Allow a Docker Host API to be used as a Cloud provider for the Jenkins server. It would do exactly what I was looking for that entire time, but avoiding the necessity of configuring a super complex Kubernetes
...
In the "Docker Host URI", fill it with the mounted socket path/address of remote Docker Host. Test Connection to ensure if Jenkins can use the Docker Cloud you've set.
...
For each template you wish to define, the following steps are important:
    + Labels: define the labels which can identify the template. This means that, if a template has the label "hello-world",
    the Cloud can provide a container agent with this template whenever a Pipeline script requires an agent with label "hello-world". (`agent {label 'hello-world'}`).
...
such as deploy tests on VMs and even physical devices. But, for now, the actual state of the integration tests can be replicated
with a simple VM running the test. 

The other tests can take advantaged of a more lightweight Docker container environment provided by the Docker Cloud.
...
the `deploy` feature testing. It probably be an even more theorical and study-focused phase of this GSoC project of mine, and I hope to keep, from now, constantly updating the 
progress and accumulated insigth on this blog.
